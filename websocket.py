...
from constants import PING_INTERVAL, PING_TIMEOUT, MAX_WEBSOCKETS, WS_TOPICS_LIMIT, WS_TOPICS_BATCH_SIZE
from utils import (
    CHARS_ASCII,
    task_wrapper,
    create_nonce,
...
class Websocket:
    ...
    async def _handle_topics(self):
        ...
        if removed:
            ...
            # send in batches
            for i in range(0, len(topics_list), WS_TOPICS_BATCH_SIZE):
                batch = topics_list[i : i + WS_TOPICS_BATCH_SIZE]
                await self.send(
                    {
                        "type": "UNLISTEN",
                        "data": {
